...
type FoodCheckDetailsProps={choices:DietChoices;onBack:()=>void};

export default function FoodCheckDetails({choices,onBack}:FoodCheckDetailsProps){
 ...
 return <main className="food-details-page" aria-labelledby="food-details-title">
  <header className="food-details-header app-page-header">
   ...
   <h1 id="food-details-title">Why can’t I eat this?</h1>
  </header>
  <div className="food-details-content">
   ...
   <section className="food-details-copy" aria-labelledby="food-details-why">
    <h2 id="food-details-why">Why can’t eat?</h2>
    <p>{matchedChoices.length?'This dish conflicts with food preferences you saved. Check the matched items before eating.':'The demo cannot confirm that this dish meets your saved food preferences.'}</p>
    {!!matchedChoices.length&&<div className="food-details-tags" aria-label="Matched reasons">{matchedChoices.map(label=><span key={label}>{label}</span>)}</div>}
   </section>
  </div>
 </main>;
}
