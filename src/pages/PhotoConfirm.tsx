...
export default function PhotoConfirm({onBack,onRetake,captureBounds,choices}:PhotoConfirmProps){
 ...
 useLayoutEffect(()=>{
  ...
  root.querySelectorAll<HTMLElement>('.confirm-header,.confirm-review-guidance,.journey-character,.confirm-actions').forEach((el,i)=>{
 ...
 const retake=()=>{
  ...
  if(!root||!captureBounds||matchMedia('(prefers-reduced-motion: reduce)').matches){onRetake();return;}
  setDeparting(true);
  const photo=root.querySelector<HTMLElement>('.confirm-photo')!;
  const pageBox=root.getBoundingClientRect();
  const animations:Animation[]=[];
  root.querySelectorAll<HTMLElement>('.confirm-header,.confirm-review-guidance,.journey-character,.confirm-actions').forEach((element,i)=>{
   animations.push(element.animate([
    {opacity:getComputedStyle(element).opacity,transform:getComputedStyle(element).transform},
    {opacity:0,transform:`translateY(${i<2?-8:10}px)`},
   ],{duration:260,delay:i*35,fill:'both',easing:'cubic-bezier(.4,0,1,1)'}));
  });
  const photoAnimation=photo.animate([
   {left:`${photo.offsetLeft}px`,top:`${photo.offsetTop}px`,width:`${photo.offsetWidth}px`,height:`${photo.offsetHeight}px`,borderRadius:getComputedStyle(photo).borderRadius},
   {left:`${captureBounds.left-pageBox.left}px`,top:`${captureBounds.top-pageBox.top}px`,width:`${captureBounds.width}px`,height:`${captureBounds.height}px`,borderRadius:'0px'},
  ],{duration:760,delay:140,fill:'both',easing:'cubic-bezier(.4,0,.2,1)'});
  animations.push(photoAnimation);
  photoAnimation.finished.then(onRetake).catch(()=>{});
 };
 return <><main className={`confirm-page journey-page phase-${phase}`} aria-labelledby="photo-confirm-title" ref={pageRef} inert={arriving||departing?true:undefined} data-arriving={arriving} data-departing={departing}>
